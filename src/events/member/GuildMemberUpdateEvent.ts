...
export class GuildMemberUpdateEvent extends AbstractEvent<GuildMemberUpdateEventContext> {
  ...
  async execute(shardId: number, data: RawGuildMemberData & { guild_id: string }) {
    ...
    const stored = await this.app.members.cache.get(data.user.id, { storage: data.guild_id })

    let updated: GuildMember
    if (stored) {
      updated = await stored._clone().then((member) => member.init(data))
    } else {
      const guildOwner = await this.app[otherCacheSymbol].get(data.guild_id, { storage: 'guild-owners' })
      const isOwner = guildOwner?.id === data.user.id

      updated = await new GMember(this.app).init({ ...data, guild_owner: isOwner })
    }

    await this.app.members.cache.set(data.user.id, updated, { storage: data.guild_id })
    ...
    this.app.emit(EventNames.GUILD_MEMBER_UPDATE, context)
    return context
  }
}