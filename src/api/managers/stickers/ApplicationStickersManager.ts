...
export class ApplicationStickersManager extends EntitiesManager {
  ...
  async fetch(sticker: StickerResolvable): Promise<Sticker | undefined> {
    ...
    if (!stickerId) throw new DiscordooError('ApplicationStickersManager#fetch', 'Cannot fetch sticker without guild id.')

    const response = await this.app.internals.actions.getSticker(stickerId)
    const Sticker = EntitiesUtil.get('Sticker')

    if (response.success) {
      const sticker = await new Sticker(this.app).init(response.result)
  ...
  async fetchMany(guild: GuildResolvable): Promise<Collection<string, Sticker> | undefined> {
    ...
    if (!guildId) throw new DiscordooError('ApplicationStickersManager#fetchMany', 'Cannot fetch guild stickers without guild id.')

    const response = await this.app.internals.actions.getGuildStickers(guildId)
    const Sticker = EntitiesUtil.get('Sticker')
    ...
    if (response.success) {
      ...
      for await (const data of response.result) {
        const sticker = await new Sticker(this.app).init(data)
  ...
  async fetchPacks(): Promise<Collection<string, StickerPack> | undefined> {
    const response = await this.app.internals.actions.getNitroStickerPacks(),
      collection = new Collection(),
      StickerPack = EntitiesUtil.get('StickerPack')

    if (response.success) {
      for await (const stickerPackData of response.result) {
        const stickerPack = await new StickerPack(this.app).init(stickerPackData)
  ...
  async edit<R = Sticker>(
    ...
    if (!stickerId) throw new DiscordooError('ApplicationStickersManager#edit', 'Cannot edit sticker without id.')
    if (!guildId) throw new DiscordooError('ApplicationStickersManager#edit', 'Cannot edit sticker without guild id.')
    ...
    const response = await this.app.internals.actions.editGuildSticker(guildId, stickerId, payload, options.reason)
    const Sticker = EntitiesUtil.get('Sticker')
    ...
    if (response.success) {
      if (options.patchEntity) {
        ...
        return await new Sticker(this.app).init(response.result) as any
      }
    }
  ...
  async create(guild: GuildResolvable, data: StickerCreateData | RawStickerCreateData, reason?: string): Promise<Sticker | undefined> {
    ...
    if (!guildId) throw new DiscordooError('ApplicationStickersManager#create', 'Cannot create sticker without guild id.')
    ...
    const response = await this.app.internals.actions.createGuildSticker(guildId, payload, reason)
    const Sticker = EntitiesUtil.get('Sticker')
  ...
  async delete(guild: GuildResolvable, sticker: StickerResolvable, reason?: string): Promise<boolean> {
    ...
    if (!stickerId) throw new DiscordooError('ApplicationStickersManager#delete', 'Cannot delete sticker without id.')
    if (!guildId) throw new DiscordooError('ApplicationStickersManager#delete', 'Cannot delete sticker without guild id.')

    const response = await this.app.internals.actions.deleteGuildSticker(guildId, stickerId, reason)
