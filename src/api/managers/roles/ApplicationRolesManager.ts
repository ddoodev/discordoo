...
export class ApplicationRolesManager extends EntitiesManager {
  ...
  async fetch(guild: GuildResolvable): Promise<Role[] | undefined> {
    ...
    if (!guildId) throw new DiscordooError('ApplicationRolesManager#fetch', 'Cannot fetch roles without guild id.')

    const response = await this.app.internals.actions.getGuildRoles(guildId)
  ...
  async create(
    ...
    if (!guildId) throw new DiscordooError('ApplicationRolesManager#create', 'Cannot create role without guild id.')

    let payload: RawRoleCreateData = {}

    payload = await ApplicationRolesManager._createPayload(payload, data)

    const response = await this.app.internals.actions.createGuildRole(guildId, payload, reason)
    const Role = EntitiesUtil.get('Role')

    if (response.success) {
      return await new Role(this.app).init({ ...response.result, guild_id: guildId })
    }
  ...
  async edit<R = Role>(
    ...
    if (!roleId) throw new DiscordooError('ApplicationRolesManager#edit', 'Cannot edit role without id.')
    if (!guildId) throw new DiscordooError('ApplicationRolesManager#edit', 'Cannot edit role without guild id.')

    let payload: RawRoleEditData = {}

    payload = await ApplicationRolesManager._createPayload(payload, data)

    const response = await this.app.internals.actions.editGuildRole(guildId, roleId, payload, options.reason)
    const Role = EntitiesUtil.get('Role')
    ...
    if (response.success) {
      if (options.patchEntity) {
        ...
        return await new Role(this.app).init(response.result) as any
      }
    }
  ...
  async delete(guild: GuildResolvable, role: RoleResolvable, reason?: string): Promise<boolean> {
    ...
    if (!roleId) throw new DiscordooError('ApplicationRolesManager#delete', 'Cannot delete role without id.')
    if (!guildId) throw new DiscordooError('ApplicationRolesManager#delete', 'Cannot delete role without guild id.')

    const response = await this.app.internals.actions.deleteGuildRole(guildId, roleId, reason)
